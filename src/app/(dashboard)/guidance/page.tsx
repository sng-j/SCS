...
import {
  BookOpen, Shield, HelpCircle, MessageSquare,
  ChevronDown, CheckCircle, ArrowRight, Layers,
  FileText, ClipboardCheck, Send, Plus,
} from "lucide-react";
...
function FaqTab({ locale }: { locale: string }) {
  ...
  const [expanded, setExpanded] = useState<string | null>(null);
  const { translate, translating } = useTranslate(tx(locale, "en", "ko", "ja"));
  const [translated, setTranslated] = useState<Record<number, { q: string; a: string }>>({});
  const translatedRef = useRef(false);

  useEffect(() => {
    fetch("/api/faq").then(async (r) => { if (r.ok) { const d = await r.json(); setFaqs(Array.isArray(d) ? d : []); }; }).finally(() => setLoading(false));
  }, []);

  // Auto-translate when English and data loaded
  useEffect(() => {
    if (locale === "ko" || faqs.length === 0 || translatedRef.current) return;
    translatedRef.current = true;
    const texts = faqs.flatMap((f) => [f.question, f.answer]);
    translate(texts).then((results) => {
      const map: Record<number, { q: string; a: string }> = {};
      faqs.forEach((f, i) => { map[f.id] = { q: results[i * 2], a: results[i * 2 + 1] }; });
      setTranslated(map);
    });
  }, [locale, faqs, translate]);

  if (loading) return <SkeletonTable rows={5} />;
  if (faqs.length === 0) return <EmptyState icon={HelpCircle} title={tx(locale, "No FAQ entries yet", "FAQ가 없습니다", "FAQはまだありません")} />;

  return (
    <div className="space-y-2">
      {locale !== "ko" && translating && (
        <p className="text-body-xs text-text-tertiary text-center py-2">Translating...</p>
      )}
      {faqs.sort((a, b) => a.sortOrder - b.sortOrder).map((faq, i) => {
        const q = translated[faq.id]?.q || faq.question;
        const a = translated[faq.id]?.a || faq.answer;
        return (
        <motion.div key={faq.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: i * 0.04 }}>
          <Card padding="none">
            <button onClick={() => setExpanded(expanded === String(faq.id) ? null : String(faq.id))} className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-surface-secondary/20 transition-colors rounded-[8px]" aria-expanded={expanded === String(faq.id)}>
              ...
              <p className="flex-1 text-body-sm font-semibold text-text">{q}</p>
              <motion.div animate={{ rotate: expanded === String(faq.id) ? 180 : 0 }} transition={{ duration: 0.2 }} className="shrink-0">
                <ChevronDown size={16} className="text-text-tertiary" />
              </motion.div>
            </button>
            <AnimatePresence>
              {expanded === String(faq.id) && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                  <div className="px-5 pb-5 pt-1 border-t border-border flex items-start gap-3">
                    <div className="h-7 w-7 rounded-full bg-green-50 flex items-center justify-center shrink-0 mt-0.5">
                      <span className="text-[11px] font-bold text-safety-low">A</span>
                    </div>
                    <p className="text-body-xs text-text-secondary leading-relaxed mt-1">{a}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
...
function QnaTab({ locale }: { locale: string }) {
  const { data: session } = useSession();
  const userRole = (session?.user as { role?: string })?.role;
  const canAnswer = userRole === "ADMIN" || userRole === "SHIPYARD";

  const [qnas, setQnas] = useState<QnaEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [form, setForm] = useState({ title: "", content: "" });
  const [saving, setSaving] = useState(false);
  const [answerTarget, setAnswerTarget] = useState<QnaEntry | null>(null);
  const [answerText, setAnswerText] = useState("");
  const [answerSaving, setAnswerSaving] = useState(false);
  const { translate, translating } = useTranslate(tx(locale, "en", "ko", "ja"));
  const [translated, setTranslated] = useState<Record<string, { q: string; a: string | null }>>({});
  const translatedRef = useRef(false);
    ...
    fetch("/api/qna")
      .then(async (r) => {
        if (r.ok) {
          ...
          const mapped = (Array.isArray(data) ? data : []).map((q: { id: string; title: string; content: string; answer: string | null; status?: string; user?: { name: string }; createdAt: string }) => ({
            id: q.id,
  ...
  // Auto-translate Q&A when English
  useEffect(() => {
    if (locale === "ko" || qnas.length === 0 || translatedRef.current) return;
    translatedRef.current = true;
    const texts = qnas.flatMap((q) => [q.question, q.answer || ""]);
    translate(texts).then((results) => {
      const map: Record<string, { q: string; a: string | null }> = {};
      qnas.forEach((q, i) => { map[q.id] = { q: results[i * 2], a: q.answer ? results[i * 2 + 1] : null }; });
      setTranslated(map);
    });
  }, [locale, qnas, translate]);

  async function handleCreate() {
    if (!form.title.trim() || !form.content.trim()) {
      showToast.error(tx(locale, "Title and content required", "제목과 내용을 입력하세요", "タイトルと内容を入力してください"));
      return;
    }
    setSaving(true);
    try {
      ...
      if (res.ok) {
        ...
        showToast.error(tx(locale, "Submit failed", "등록 실패", "登録失敗"));
      }
  ...
  async function handleAnswer() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast.error(tx(locale, "Failed to answer", "답변 실패", "回答失敗"));
      }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => setDialogOpen(true)}>
          <Plus size={14} /> {tx(locale, "Ask Question", "질문하기", "質問する")}
        </Button>
      </div>

      {loading ? <SkeletonTable rows={4} /> : qnas.length === 0 ? (
        <EmptyState
          icon={MessageSquare}
          title={tx(locale, "No Q&A yet", "Q&A가 없습니다", "Q&Aはまだありません")}
          subtitle={tx(locale, "Ask a question to get started", "궁금한 점이 있으면 질문을 등록하세요", "質問を登録して始めましょう")}
        />
      ) : (
        qnas.map((qna, i) => {
          const tq = translated[qna.id]?.q || qna.question;
          const ta = translated[qna.id]?.a || qna.answer;
          return (
          <motion.div key={qna.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: i * 0.05 }}>
            <Card padding="none">
              <CardBody>
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <p className="text-body-sm font-semibold text-text">{tq}</p>
                    <p className="text-[11px] text-text-tertiary mt-0.5">
                      {qna.authorName} · {new Date(qna.createdAt).toLocaleDateString(locale === "ko" ? "ko-KR" : locale === "ja" ? "ja-JP" : "en-US", { month: "short", day: "numeric" })}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3 pt-3 border-t border-border">
                  <div className={cn("h-7 w-7 rounded-full flex items-center justify-center shrink-0", ta ? "bg-green-50" : "bg-surface-secondary")}>
                    <span className={cn("text-[11px] font-bold", ta ? "text-safety-low" : "text-text-tertiary")}>A</span>
                  </div>
                  <div className="flex-1 flex items-start justify-between gap-2">
                    <p className={cn("text-body-xs leading-relaxed mt-1", ta ? "text-text-secondary" : "text-text-tertiary italic")}>
                      {ta || (tx(locale, "No answer yet", "아직 답변이 없습니다", "まだ回答がありません"))}
                    </p>
                    {canAnswer && (
                      <Button size="sm" variant={ta ? "ghost" : "outline"} className="shrink-0" onClick={() => { setAnswerTarget(qna); setAnswerText(qna.answer || ""); }}>
                        <Send size={13} /> {!ta && tx(locale, "Answer", "답변", "回答")}
                      </Button>
                    )}
                  </div>
                </div>
      ...
      <Dialog
        ...
        title={tx(locale, "Ask a Question", "질문하기", "質問する")}
        description={tx(locale, "Ask about E27 certification", "E27 인증 관련 궁금한 점을 질문하세요", "E27認証に関する質問をしてください")}
      >
        <div className="space-y-4">
          <Input
            label={tx(locale, "Title *", "제목 *", "タイトル *")}
            placeholder={tx(locale, "Enter question title", "질문 제목을 입력하세요", "質問タイトルを入力してください")}
            value={form.title}
            onChange={(e) => setForm({ ...form, title: e.target.value })}
          />
          <Textarea
            label={tx(locale, "Content *", "내용 *", "内容 *")}
            placeholder={tx(locale, "Describe your question in detail", "질문 내용을 상세히 작성하세요", "質問内容を詳しく記入してください")}
            rows={4}
            value={form.content}
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-border">
            ...
            <Button onClick={handleCreate} loading={saving}>
              <Send size={14} /> {tx(locale, "Submit", "등록", "登録")}
            </Button>
          </div>
        </div>
      </Dialog>

      {/* Answer Q&A Dialog (ADMIN / SHIPYARD) */}
      {canAnswer && (
        <Dialog open={!!answerTarget} onClose={() => setAnswerTarget(null)} title={tx(locale, "Answer Q&A", "Q&A 답변", "Q&A回答")}>
          <div className="space-y-4">
            <div className="p-3 rounded-lg bg-surface-secondary">
              <p className="text-body-sm font-semibold text-text">{answerTarget?.title || answerTarget?.question}</p>
            ...
            <div className="flex justify-end gap-3 pt-2 border-t border-border">
              ...
              <Button onClick={handleAnswer} loading={answerSaving}>
                <Send size={14} /> {tx(locale, "Submit Answer", "답변 등록", "回答登録")}
              </Button>
            </div>
          </div>
        </Dialog>
